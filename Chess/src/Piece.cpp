...
/**
 * @brief Converts potential move destinations into Move objects representing legal moves.
 *
 * Iterates over the potential destination boxes returned by getPotentialMoves(),
 * creates a Move from the current location to each destination, and collects them.
 *
 * @return std::vector<Move> Vector of legal moves available for this piece.
 */
...
/**
 * @brief Adds new potential move destinations to the piece's internal potential moves set.
 *
 * This method inserts the provided vector of potential move destinations into the
 * piece's `_potential_moves` set to keep track of where the piece can move next.
 *
 * @param potentialMoves Vector of boxes representing potential move destinations.
 */
void Piece::setPotentialMoves(const std::vector<Box> &potentialMoves) {
    _potential_moves.insert(potentialMoves.begin(),potentialMoves.end());
}
