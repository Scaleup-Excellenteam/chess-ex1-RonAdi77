...
public:
    ...
    */
    ...
    /**
     * @brief Retrieves all pieces of the specified color currently on the board.
     *
     * @param color The color (WHITE or BLACK) of pieces to retrieve.
     * @return std::vector<std::shared_ptr<Piece>> Vector of shared pointers to the pieces.
     */
    std::vector<std::shared_ptr<Piece>> getPieces(COLOR);
    /**
     * @brief Checks if the current game state is a draw.
     *
     * This can include insufficient material, stalemate, threefold repetition, etc.
     *
     * @return true if the game is drawn; otherwise, false.
     */
    bool isDraw();
    /**
     * @brief Checks if the game is drawn by threefold repetition.
     *
     * Detects if the same board position has occurred three times with the same player to move.
     *
     * @return true if a threefold repetition draw has occurred; otherwise, false.
     */
    bool isThreeFoldDraw();
    /**
     * @brief Checks if the player of the specified color is in stalemate.
     *
     * A stalemate occurs when the player has no legal moves and is not in check.
     *
     * @param color The color of the player to check.
     * @return true if the player is stalemated; otherwise, false.
     */
    [[nodiscard]] bool isStalemate(COLOR) const;
    /**
     * @brief Returns a string representation of the current board state.
     *
     * Typically used for debugging or displaying the board to the user.
     *
     * @return std::string String visualization of the board.
     */
    [[nodiscard]] std::string boardToString() const;


private:
    std::map<Box,std::shared_ptr<Piece>> _board;
    std::vector<std::map<Box,std::shared_ptr<Piece>>> _boardHistory;
    ...
    */
    ...
    /**
    * @brief Performs a deep copy of a given board map.
    * @param sourceBoard The map to be copied.
    * @return A new map containing deep copies of the pieces.
    */
    [[nodiscard]] std::map<Box, std::shared_ptr<Piece>> copyBoard(const std::map<Box, std::shared_ptr<Piece>>& sourceBoard) const;
