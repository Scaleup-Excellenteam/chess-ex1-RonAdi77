...
public:
    ...
    void setPotentialMoves(const  std::vector<Box>  &potentialMoves);
   ...
   */
    ...
    /**
     * @brief Returns all possible legal moves based on current board state and movement rules.
     *
     * Only moves that are valid according to the game rules (no self-check, etc.) are included.
     *
     * @return std::vector<Move> Vector of legal candidate moves.
     */
    std::vector<Move> getPotenMoves();
    /**
     * @brief Returns the character representation of the piece.
     *
     * This is a pure virtual method that must be implemented by derived piece classes
     * to return a single character representing the piece type (e.g., 'K' for King).
     *
     * @return char Character representing the piece.
     */
